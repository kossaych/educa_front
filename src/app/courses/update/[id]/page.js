...
function UpdateCourse(props) {
  ...
  useEffect(()=>{
    fetch("https://kossaych.pythonanywhere.com/content/api/course_pk/" + +(props.params.id), {
      method: "get",
      headers: {
        "Content-Type": "application/json",
        'Authorization': 'token ' + JSON.parse(localStorage.getItem('token')),
   ...
   const [newVideo,setNewVideo] = useState('')
    ...
    const handleAddVideo = () => {
      ...
      fetch("https://kossaych.pythonanywhere.com/content/api/video/" , {
        method: "post",
        headers: { 
          'Authorization': 'token ' + JSON.parse(localStorage.getItem('token')),
                                            ...
                                            <button    className="hidden m-auto  rounded-lg p-2  bg-green-200"  > <CiBoxList size={25}></CiBoxList> </button>  
                                                     ...
                                                     <ul class="h-48 px-3 pb-3 overflow-y-auto text-sm text-blue-700 dark:text-gray-200" aria-labelledby="dropdownSearchButton">
                                                        ...
                                                        {course.levels.map(level => (  
                                                            <li key={level.id}>
                                                              <div key={level.id} class="flex items-center p-2 rounded ">
                                                                <input   type="checkbox" value="" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500"   id={level.id} onClick={(e) => {handleLevelSelection(e)}} />  
                                                                <label   class="w-full ms-2 text-sm font-medium text-gray-900 rounded  ">{level.title}</label>
                                                              </div>
                                                            </li> 
                                                            ))}
